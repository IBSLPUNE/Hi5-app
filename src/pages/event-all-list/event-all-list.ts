...
/**
 * Generated class for the EventAllListPage page.
 *
...
export class EventAllListPage {
 ...
 date_Max1: any;
  ...
  getalleventdetails(){
   this.showLoader();
            this.authService.getalleventdetails(this.member_type.value, localStorage.getItem('reporting_id'), moment(this.selectedDate).format('YYYY-MM-DD'), moment(this.selectedDate1).format('YYYY-MM-DD')).then((result) => {
    ...
    // geteventcomplete(event_detail_id){
     // this.showLoader();
 ...
 showLoader() {
   ...
   showModalDialog(req_data) {
     ...
     showNewEventRequest(){
      var EventRequestPagemodalPage = this.modalCtrl.create('EventDetailsRequestPage');
