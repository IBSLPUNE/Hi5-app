...
/**
 * Generated class for the PurticularEventDetailsRequestPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-purticular-event-details-request',
  templateUrl: 'purticular-event-details-request.html',
})
export class PurticularEventDetailsRequestPage {
  ...
  // EventDate: any;
   ...
   EventReq = { county_type: '', state_type: '', district_type: '', event_type: '', code: '', name: '', discription: '', current_date: '', place: '', town: '', village: '', city_type: '', member_type:'', agency_type:'', select_type: '', data_collections: ''}
 ...
 constructor(public navCtrl: NavController, public navParams: NavParams, private alertCtrl: AlertController, private authService: AuthServiceProvider, public loadingCtrl: LoadingController, private toastCtrl: ToastController, public viewCtrl: ViewController) {
   ...
   setDate(date: Date) {
    ...
    //onLTChange() {
     //   if (this.select_type.value == 'Listed') {
          
      //      this.lTType = false;
      //  }
      //  if (this.select_type.value == 'Unlisted') {
   ...
   // }
    ...
    getDataCollections() {
      //  this.showLoader();
        this.authService.getDataCollections(localStorage.getItem('agent_id')).then((result) => {
           // this.loading.dismiss();

            this.datacollections = result;

            this.data_collections_data.push({ value: '', text: 'Select Data' });

            for (let i = 0; i < this.datacollections.length; i++) {
                this.data_collections_data.push({ value: this.datacollections[i].id, text: this.datacollections[i].contact_name });
            }

            this.data_collections = { text: 'Select Data', value: '' };
       ...
       // this.showLoader();
        this.authService.getpurticularmemberdetails(localStorage.getItem('agent_id')).then((result) => {
           // this.loading.dismiss();
            ...
            this.member_type_data.push({ value: '', text: 'Select Member' });

            for (let i = 0; i < this.membertype.length; i++) {
                this.member_type_data.push({ value: this.membertype[i].id, text: this.membertype[i].first_name + ' ' + this.membertype[i].middle_name + ' ' + this.membertype[i].last_name });
            }

            this.member_type = { text: 'Select Member', value: '' };
    ...
    saveEventData(){
        ...
        if (this.Town == '') {
             this.showAlert('Message', 'Town is Mandatory.');
            return;
        }
        if (this.event_type.value == '') {
             this.showAlert('Message', 'Please Select Event Categories.');
            return;
        }
       ...
       // this.EventReq.select_type = this.select_type.value;
        this.EventReq.data_collections = this.data_collections.value;
       ...
       // this.EventReq.district_type = this.district_type.value;
        ...
        this.EventReq.member_type = localStorage.getItem('agent_id');
